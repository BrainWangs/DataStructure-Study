...
int GetElem(LinkList L, int i, int *e) {
    ...
    int j = 1;
    while(p && j < i) { //当p指向的不是空 且 计数器j小于目标点i时 当j=i-1时执行并退出
        p = p -> next; //使p向后移动一位
        j++;
    }
    if(!p || j > i) {
...
/*查找*/        /*根据给定值e比较并返回指针*/
LNode *LocateElem(LinkList L, int e) {  //理论上不使用二级指针好像也能正常运行
    LinkList p = L -> next;
    while(p -> data != e && p != NULL) { //考虑p为null的情况
        p = p -> next;
    }
    if(p == NULL) { //若p因为空而退出while,提示错误信息
        printf("Error");
        return NULL;
    }
    return p;
...
int ListInsert(LinkList *L, int i, int e) { 
    LinkList p = *L;
    int j = 0;
    while(p && j < i - 1) { //在位置i插入,指针p应该指向i-1的位置
        p = p -> next;
        j++;
    }
    if(!p || j > i - 1) { //i<0时触发
...
// int CreaList_H(LinkList *L, int size) {
//     if(! *L) {
//         printf("Error\n");
//         return Error;
//     }
